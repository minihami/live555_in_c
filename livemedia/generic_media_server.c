		...
		uv_tcp_t *our_socket, groupsock_port_t our_port, unsigned int reclamation_seconds)
{
	...
	uv_fileno((uv_handle_t *)generic_media_server->server_socket, &socket_fd);
	groupsock__ignore_sig_pipe_on_socket(socket_fd);
...
{
	if (generic_media_server && generic_media_server->lookup_server_media_session)
...
{
	if (generic_media_server &&
			generic_media_server->create_new_client_connection)
...
{
	if (generic_media_server &&
			generic_media_server->create_new_client_session)
		...
		bool is_first_lookup_in_session)
{
	...
	return (livemedia_server_media_session_t *)livemedia_hash_table__lookup(
			generic_media_server->server_media_sessions, stream_name);
}
		...
		livemedia_generic_media_server_t *generic_media_server, char const *stream_name)
{
	...
	livemedia_generic_media_server__remove_server_media_session_by_server_media_session(
			generic_media_server, livemedia_generic_media_server__lookup_server_media_session(
				generic_media_server, stream_name, true /* empty parameter */));
}
		...
		livemedia_generic_media_server_t *generic_media_server, char const *stream_name)
{
	...
	livemedia_generic_media_server__close_all_client_sessions_for_server_media_session_by_server_media_session(
			generic_media_server,
			livemedia_generic_media_server__lookup_server_media_session(
				generic_media_server, stream_name, true /* empty parameter */));
}
...
uv_tcp_t *livemedia_generic_media_server__set_up_our_socket__static(groupsock_port_t *our_port)
{
	...
	do {
		...
		if (our_socket == NULL)
		{
			printf("our socket NULL %s_()\n", __func__);
			break;
		}
		uv_fileno((uv_handle_t *)our_socket, &socket_fd);
		if (!groupsock__increase_send_buffer_to(socket_fd, 50*1024))
		{
			printf("groupsock__increase_send_buffer_to false %s_()\n", __func__);
			break;
		}
      ...
      /* bind() will have chosen a port for us; return it also: */
			if (!groupsock__get_source_port(socket_fd, our_port))
				break; 
		}
		return our_socket;
	} while (0);
		...
		livemedia_generic_media_server_t *generic_media_server, uv_tcp_t *server_socket)
{
	...
	if (client_socket == NULL) {
		fprintf(stderr, "malloc(sizeof(uv_tcp_t))"
				"failed in %s()\n", __func__);
		uv_close((uv_handle_t *)client_socket, on_close_cb);
		return;
	}
	...
	uv_fileno((uv_handle_t *)client_socket, &socket_fd);
	groupsock__ignore_sig_pipe_on_socket(socket_fd);
	/* so that clients on the same host that are killed don't also kill us */
	groupsock__make_socket_non_blocking(socket_fd);
	groupsock__increase_send_buffer_to(socket_fd, 50*1024);
#ifdef DEBUG
		...
		livemedia_generic_media_server_t *generic_media_server)
{
  ...
  * session id twice in a row.) */
	...
	do {
		session_id = (u_int32_t)our_random32();
		snprintf(session_id_str, sizeof(session_id_str), "%08X", session_id);
	} while (session_id == 0 || 
			session_id == generic_media_server->previous_client_session_id ||
			livemedia_generic_media_server__lookup_client_session_by_session_id_str(
				generic_media_server, session_id_str) != NULL);
		...
		livemedia_generic_media_server_t *our_server, u_int32_t session_id)
{
	...
	generic_media_server_client_session->liveness_check_task = 
		(uv_timer_t *)calloc(1, sizeof(uv_timer_t));
	if (generic_media_server_client_session->liveness_check_task == NULL) {
		fprintf(stderr, "calloc(sizeof(uv_timer_t))"
				"failed in %s()\n", __func__);
	}
	uv_timer_init(uv_default_loop(), generic_media_server_client_session->liveness_check_task);
		...
		livemedia_generic_media_server_client_session_t *generic_media_server_client_session)
{
	...
	if (generic_media_server_client_session->liveness_check_task) {
		uv_timer_stop(generic_media_server_client_session->liveness_check_task);
		uv_close((uv_handle_t *)generic_media_server_client_session->liveness_check_task,
				on_timer_close_cb);
	}
 ...
 * Normal functions
 */
static void livemedia__note_liveness_cb(uv_timer_t *handle)
{
	livemedia_generic_media_server_client_session_t *generic_media_server_client_session;
	generic_media_server_client_session = 
		(livemedia_generic_media_server_client_session_t *)handle->data;

	livemedia_generic_media_server_client_session__liveness_timeout_task__static(
			generic_media_server_client_session);
}
		...
		livemedia_generic_media_server_client_session_t *generic_media_server_client_session)
{
	...
	if (generic_media_server_client_session->our_server->reclamation_seconds > 0) {
		uv_timer_stop(generic_media_server_client_session->liveness_check_task);
		generic_media_server_client_session->liveness_check_task->data =
			(void *)generic_media_server_client_session;
		uv_timer_start(generic_media_server_client_session->liveness_check_task,
				livemedia__note_liveness_cb,
				generic_media_server_client_session->our_server->reclamation_seconds * 1000,
				0);
	}
}
		...
		char const *username)
{
	if (user_authentication_database && user_authentication_database->lookup_password)
		return user_authentication_database->lookup_password(user_authentication_database,
				username);
	return NULL;
}
